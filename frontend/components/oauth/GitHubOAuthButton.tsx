...
export default function GitHubOAuthButton({ className = "" }: GitHubOAuthButtonProps) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className={`w-full flex items-center justify-center gap-3 px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white font-medium hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
      {loading ? (
