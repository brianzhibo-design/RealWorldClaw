...
import { CommunityPost, apiFetch } from "@/lib/api-client";
...
export default function ProfilePage() {
  ...
  const [userNodes, setUserNodes] = useState<ProfileNode[]>([]);
  ...
  const loadProfileCore = useCallback(async () => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const posts = await apiFetch<CommunityPost[]>(`/community/posts?author_id=${encodeURIComponent(userId)}`)
        .catch(() => []);
      setUserPosts(posts);

      if (!isOwnProfile && posts.length > 0) {
        setProfileName(posts[0].author_name || posts[0].author || "");
      }
  ...
  const loadOwnResources = useCallback(async () => {
    if (!isOwnProfile) {
      setUserNodes([]);
      setUserOrders([]);
      return;
    ...
    try {
      ...
      const nodesRes = await apiFetch<ProfileNode[] | { nodes?: ProfileNode[] }>("/nodes/my-nodes");
      setUserNodes(Array.isArray(nodesRes) ? nodesRes : nodesRes.nodes || []);
    } catch (err) {
    ...
    try {
      ...
      const ordersRes = await apiFetch<ProfileOrder[] | { orders?: ProfileOrder[] }>("/orders");
      setUserOrders(Array.isArray(ordersRes) ? ordersRes : ordersRes.orders || []);
    } catch (err) {
  ...
  useEffect(() => {
    loadProfileCore();
  }, [loadProfileCore, refreshKey]);

  useEffect(() => {
    loadOwnResources();
  }, [loadOwnResources, refreshKey]);
    ...
    return (
      <div className="bg-slate-950 min-h-screen text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => setRefreshKey((v) => v + 1)} className="px-4 py-2 bg-sky-600 hover:bg-sky-500 hover:shadow-[0_0_18px_rgba(56,189,248,0.35)] rounded-lg transition-all">
            Retry
          </button>
        </div>
      </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-8 mb-8">
          <div className="flex items-center gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-slate-400 mt-1">{isOwnProfile && currentUser ? currentUser.role : "Member"}</p>
              ...
              {isOwnProfile ? (
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    isFollowing ? "bg-slate-700 hover:bg-sky-500 text-white" : "bg-sky-600 hover:bg-sky-500 text-white"
            ...
            <div className="space-y-4">
              {userPosts.map((post) => (
                ...
                >
                  ...
                  <h3 className="text-lg font-semibold group-hover:text-sky-400 transition-colors">{post.title}</h3>
                  <p className="text-slate-400 text-sm mt-1 line-clamp-2">{post.content.substring(0, 200)}</p>
              ...
              <div className="space-y-4">
                {userOrders.slice(0, 5).map((order) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-400 mb-2 line-clamp-1">{order.description || "No description"}</p>
